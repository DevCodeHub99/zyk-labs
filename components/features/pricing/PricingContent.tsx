...
const PricingCard = ({ plan }: { plan: PricingPlan }) => {
  const { getPlanPrice } = useCurrency()
    ...
    >
      {plan.badge && (
        <div className="absolute top-5 right-5 z-20">
          <span
            className={`text-[9px] font-black px-3 py-1 rounded-full uppercase tracking-widest whitespace-nowrap shadow-sm ${
              plan.popular ? 'bg-accent text-white' : 'bg-primary text-primary-foreground'
            }`}
          >
            {plan.badge}
          </span>
        ...
        {plan.tagline && (
          <p
            className={`text-[9px] md:text-[10px] font-black uppercase tracking-[0.2em] ${
              plan.popular ? 'text-accent' : 'text-foreground/35'
            }`}
          >
            {plan.tagline}
          </p>
      ...
      <div className="mb-6 md:mb-8 pb-6 md:pb-8 border-b border-border/40">
        {isCustom ? (
          <div>
            ...
            <p className="text-[9px] md:text-[10px] text-foreground/40 mt-1 uppercase font-bold tracking-wider">
              Custom build · Fixed scope
            </p>
          </div>
        ) : (
          <div>
            <div className="flex items-baseline gap-1">
              <span className="text-3xl md:text-4xl font-black tracking-tighter text-primary">
                {displayPrice}
              </span>
              {plan.period && (
                <span className="text-[10px] md:text-xs text-foreground/40 font-black uppercase tracking-widest">
                  {plan.period}
                </span>
              )}
            </div>
            ...
            <li key={i} className="flex items-start gap-3">
              <div
                className={`flex-shrink-0 w-5 h-5 rounded-lg flex items-center justify-center ${
                  plan.popular ? 'bg-accent/10 text-accent' : 'bg-secondary/60 text-foreground/40'
                }`}
              >
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-16 md:py-32">
        <div className="flex flex-col items-center">
          <SectionHeader
            ...
            title={
              <>
                {pricing.titleHighlighted.main} <br />
                <span className="text-accent italic-serif">{pricing.titleHighlighted.accent}</span>
              </>
            }
          ...
          <div className="mt-8 flex items-center gap-3">
            <span className="text-[10px] font-black uppercase tracking-widest text-foreground/40">Select Currency:</span>
        ...
        <ContactCTA
          title={
            <>
              Have a unique <br />
              <span className="text-accent italic-serif">product requirement?</span>
            </>
          }
