          ...
          <div className="space-y-8 text-center lg:text-left">
            <div className="space-y-6">
              ...
              <h1 className="text-4xl md:text-6xl lg:text-7xl font-bold tracking-tight text-primary leading-[1.1]">
                {hero.title.first} <br />
                {hero.title.second} <span className="text-accent">{hero.title.highlight}</span>
              </h1>
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center lg:justify-start">
              <Button asChild size="lg" className="h-12 px-8 text-base bg-accent hover:bg-accent/90 text-white shadow-lg shadow-accent/25 transition-all hover:scale-105">
                <a href={hero.cta.primary.href} className="flex items-center gap-2">
                  {hero.cta.primary.text} <ArrowRight className="w-4 h-4" />
                </a>
              </Button>
              <Button
                asChild
              ...
              >
                <a href={hero.cta.secondary.href} className="flex items-center gap-2">
                  {hero.cta.secondary.text}
                </a>
              </Button>
            </div>

            <div className="pt-8 flex items-center justify-center lg:justify-start gap-8 text-sm text-foreground/70">
              <div className="flex items-center gap-2">
                <svg className="w-5 h-5 text-accent" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                </svg>
                <span>{hero.usp[0].text}</span>
              </div>
              <div className="flex items-center gap-2">
                <svg className="w-5 h-5 text-accent" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z" />
                </svg>
                <span>{hero.usp[1].text}</span>
              </div>
            </div>
          </div>
