  ...
  return (
    <section id="projects" className="py-20 md:py-32 bg-transparent relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="grid md:grid-cols-3 gap-6 mb-20 max-w-6xl mx-auto md:mx-0">
          {items.slice(0, 3).map((item) => (
            <StudioCard 
              key={item.title} 
              className="p-6 transition-all duration-300 group flex flex-col h-full"
              innerClassName="flex flex-col h-full"
            >
              <div className="flex justify-between items-start mb-4">
                <h3 className="text-xl font-bold tracking-tight text-primary group-hover:text-accent transition-colors">{item.title}</h3>
                {item.link && (
                  <a 
                    href={item.link} 
                    target="_blank" 
                    rel="noopener noreferrer"
                    className="w-8 h-8 rounded-full bg-secondary flex items-center justify-center text-foreground/50 group-hover:bg-accent group-hover:text-white transition-all shadow-sm"
              ...
              <div className="mt-8 pt-4 border-t border-border/50">
                <p className="text-[10px] font-bold text-foreground/40 tracking-[0.15em] uppercase">
                  {item.tech.join(' + ')}
                </p>
              </div>
            </StudioCard>
        ...
        {featuredCaseStudy && (
          <StudioCard className="max-w-6xl p-0 overflow-hidden border-none shadow-2xl" innerClassName="bg-primary flex flex-col lg:flex-row items-stretch min-h-[400px]">
            ...
            <div className="lg:w-1/2 p-8 md:p-12 lg:p-16 space-y-6 relative z-10 text-primary-foreground">
              <span className="inline-block text-xs font-black tracking-widest uppercase text-accent bg-accent/10 px-3 py-1 rounded-full">
                {featuredCaseStudy.badge}
              </span>
              
              <h3 className="text-3xl md:text-5xl font-black leading-tight tracking-tight italic-serif">
                {featuredCaseStudy.title}
              </h3>
              
              <div className="space-y-4 pt-4 text-sm md:text-base text-primary-foreground/80 leading-relaxed font-medium">
               ...
               <div className="pt-8">
                <Button size="lg" className="rounded-full bg-accent hover:opacity-90 text-white font-black uppercase tracking-widest text-[11px] h-14 px-8 shadow-xl shadow-accent/20 group" asChild>
            ...
            <div className="lg:w-1/2 w-full min-h-[300px] md:min-h-[400px] overflow-hidden relative shadow-inner z-10 border-l border-white/5 bg-black/20 backdrop-blur-3xl">
